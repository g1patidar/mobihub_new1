...
const AddCoupon = () => {
  ...
  //   console.log(input)
  //   // try {
  //   //   axios.post("https://localhost:5000/api/user/AddCoupon", input).then(() => {
  ...
  const createcoupon = async () => {
    ...
    try {
      await axios.post("https://localhost:5000/api/user/AddCoupon", input);
      toast.success("Coupon Successfully Added!");
      Mycoupon();
      setInput({
        Coupon_Name: "",
  ...
  const Mycoupon = async () => {
    await axios.post("https://localhost:5000/api/user/DisplayCoupons").then((res) => {
  ...
  const handleDelete = async (couponid) => {
    try {
      ...
      axios.delete(`https://localhost:5000/api/user/DeleteCoupon/${couponid}`);
